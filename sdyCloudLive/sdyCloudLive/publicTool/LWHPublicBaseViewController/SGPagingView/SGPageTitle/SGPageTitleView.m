...
- (void)addBadgeForIndex:(NSInteger)index withText:(NSString *)text {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        CGFloat badgeY;
        if (text.length > 0) {
            self.configure.badgeSize = 15;
            badgeY = 0;
        } else {
            badgeY = 0.5 * (btn.SG_height - btnTextHeight) + self.configure.badgeOff.y - self.configure.badgeSize;
        }
        
        CGFloat textWidth = [self P_sizeWithString:text font:self.configure.badgeTextFont].width + 6;
        CGFloat badgeWidth = textWidth < self.configure.badgeSize ? self.configure.badgeSize : textWidth > (btn.frame.size.width-badgeX) ? (btn.frame.size.width-badgeX) : textWidth;
        CGFloat badgeHeight = self.configure.badgeSize;
...
- (void)setOnlyResetSelectedIndex:(NSInteger)onlyResetSelectedIndex {
    _onlyResetSelectedIndex = onlyResetSelectedIndex;
    UIButton *button = self.btnMArr[onlyResetSelectedIndex];
    // 1、改变按钮的选择状态
    [self P_changeSelectedButton:button];
    // 2、标题滚动样式下选中标题居中处理
    if (self.allBtnWidth > self.frame.size.width) {

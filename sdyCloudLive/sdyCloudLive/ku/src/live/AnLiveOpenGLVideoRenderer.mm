...
GLuint AnLiveCreateShader(GLenum type, const GLchar* source) {
...
GLuint AnLiveCreateProgram(GLuint vertexShader, GLuint fragmentShader) {
...
@implementation AnLiveOpenGLVideoRenderer {
#if TARGET_OS_IPHONE
...
- (instancetype)initWithContext:(EAGLContext*)context isMirror:(BOOL)isMirror{
    ...
    - (BOOL)drawFrame:(RTCI420Frame*)frame {
        ...
        if (frame) {
            ...
            GLfloat cameraPosition = 1.0f;
            if(self.isMirror)
            {
                cameraPosition = -1.0f;
            }
            AnLiveGLMatrix *camera = [AnLiveGLMatrix matrix];
            [camera populateToLookAt:AnLiveGLVectorMake(0.0f, 0.0f, 0.0f) withEyeAt:AnLiveGLVectorMake(0.0f, 0.0f, cameraPosition) withUp:AnLiveGLVectorMake(0.0f, 1.0f, 0.0f)];
            glUniformMatrix4fv(_cameraUniform, 1, 0, camera.getMatrix);
...
#pragma mark - Private
    ...
    - (BOOL)setupProgram {
        ...
        GLuint vertexShader = AnLiveCreateShader(GL_VERTEX_SHADER, kVertexShaderSource);
        NSAssert(vertexShader, @"failed to create vertex shader");
        GLuint fragmentShader =
        AnLiveCreateShader(GL_FRAGMENT_SHADER, kFragmentShaderSource);
        NSAssert(fragmentShader, @"failed to create fragment shader");
        _program = AnLiveCreateProgram(vertexShader, fragmentShader);
        ...
        _cameraUniform = glGetUniformLocation(_program, "camera");

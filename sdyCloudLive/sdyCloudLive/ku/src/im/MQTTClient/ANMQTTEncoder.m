...
{
    self.status = ANMQTTEncoderStatusInitializing;
    self.stream = stream;
    ...
    switch (eventCode) {
        ...
        case NSStreamEventHasSpaceAvailable: {
            ...
            if(![self applySSLSecurityPolicy:sender withEvent:eventCode]){
                self.status = ANMQTTEncoderStatusError;
                ...
                [self.delegate encoder:self handleEvent:ANMQTTEncoderEventErrorOccurred error:sslError];
            }

            if (self.status == ANMQTTEncoderStatusInitializing) {
                self.status = ANMQTTEncoderStatusReady;
                [self.delegate encoder:self handleEvent:ANMQTTEncoderEventReady error:nil];
            }
            else if (self.status == ANMQTTEncoderStatusReady) {
                // notify handler that we are ready and waiting for send event.
                [self.delegate encoder:self handleEvent:ANMQTTEncoderEventReady error:nil];
            }
            else if (self.status == ANMQTTEncoderStatusSending) {
                UInt8* ptr;
                NSInteger n, length;
                ...
                if (n == -1) {
                    self.status = ANMQTTEncoderStatusError;
                    [self.delegate encoder:self handleEvent:ANMQTTEncoderEventErrorOccurred error:self.stream.streamError];
                }
                else if (n < length) {
                    self.byteIndex += n;
                }
        ...
        case NSStreamEventEndEncountered:
            if (self.status != ANMQTTEncoderStatusError) {
                self.status = ANMQTTEncoderStatusError;
                NSError *error = [self.stream streamError];
                [self.delegate encoder:self handleEvent:ANMQTTEncoderEventErrorOccurred error:error];
            }
            break;
        default:
...
- (void)encodeMessage:(ANMQTTMessage*)msg {
    ...
    if (self.status != ANMQTTEncoderStatusReady) {
    ...
    if (n == -1) {
        ...
        self.byteIndex += n;
        self.status = ANMQTTEncoderStatusSending;
    }
    else {
        self.buffer = NULL;

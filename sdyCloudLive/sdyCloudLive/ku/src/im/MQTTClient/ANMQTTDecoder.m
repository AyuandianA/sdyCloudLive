...
{
    self.status = ANMQTTDecoderStatusInitializing;
    self.stream = stream;
    ...
    switch (eventCode) {
        ...
        case NSStreamEventHasBytesAvailable:
            ...
            if(![self applySSLSecurityPolicy:sender withEvent:eventCode]){
                ...
                [self.delegate decoder:self handleEvent:ANMQTTDecoderEventProtocolError error:sslError];
            }

            if (self.status == ANMQTTDecoderStatusDecodingHeader) {
                UInt8 buffer;
                NSInteger n = [self.stream read:&buffer maxLength:1];
                self.header = buffer;
                if (n == -1) {
                    self.status = ANMQTTDecoderStatusConnectionError;
                    [self.delegate decoder:self handleEvent:ANMQTTDecoderEventConnectionError error:self.stream.streamError];
                } else if (n == 1) {
                    self.length = 0;
                    self.lengthMultiplier = 1;
                    self.status = ANMQTTDecoderStatusDecodingLength;
                }
            }
            while (self.status == ANMQTTDecoderStatusDecodingLength) {
                ...
                if ((digit & 0x80) == 0x00) {
                    ...
                    self.status = ANMQTTDecoderStatusDecodingData;
                } else {
                    self.lengthMultiplier *= 128;
                }
            }
            if (self.status == ANMQTTDecoderStatusDecodingData) {
                if (self.length > 0) {
                    NSInteger n, toRead;
                    UInt8 buffer[768];
                ...
                if ([self.dataBuffer length] == self.length) {
                    ...
                    msg = [[ANMQTTMessage alloc] initWithType:type
                                                        qos:qos
        ...
        case NSStreamEventEndEncountered:
            self.status = ANMQTTDecoderStatusConnectionClosed;
            [self.delegate decoder:self handleEvent:ANMQTTDecoderEventConnectionClosed error:nil];
            break;
        case NSStreamEventErrorOccurred:
        {
            self.status = ANMQTTDecoderStatusConnectionError;
            NSError *error = [self.stream streamError];
            [self.delegate decoder:self handleEvent:ANMQTTDecoderEventConnectionError error:error];
            break;
        }
        default:

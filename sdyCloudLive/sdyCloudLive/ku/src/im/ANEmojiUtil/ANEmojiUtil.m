...
@implementation ANEmojiUtil

+ (NSString *)emojiConvertToString:(NSString *)message
{
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        NSString *value = [self string2Unicode:[message substringWithRange:matchRange]];
        matchRange.location = matchRange.location + offset;
        result = [result stringByReplacingCharactersInRange:matchRange withString:value];
        offset += value.length - matchRange.length;
        
    }
    return result;
}

+ (NSString *)stringConvertToEmoji:(NSString *)message
{
    ...
    convertedString = [convertedString stringByReplacingOccurrencesOfString:@"\\u]" withString:@"\\u"];
    return convertedString;
}

+(NSString *)string2Unicode:(NSString *)text
{
